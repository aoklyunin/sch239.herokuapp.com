...
def marks(request):
    ...
    tasks = Task.objects.filter(pub_date__gt=datetime.date.today() - datetime.timedelta(days=60)).order_by('pub_date')
    # имена заданий
    tasknames = []
    # типы заданий
    tasktypes = []
...
def cheaters(request):
    ...
    for task in Task.objects.filter(task_type=tt).filter(
            pub_date__gt=datetime.date.today() - datetime.timedelta(days=50)).filter(
